<x-layout title="Submit Complaint">
    <x-slot name="header">
        <h1 class="text-2xl font-bold text-gray-900">Submit a Complaint</h1>
        <p class="mt-1 text-sm text-gray-500">Describe your concern and we'll get back to you as soon as possible.</p>
    </x-slot>
        ...
        <form method="POST" action="{{ route('user.complaints.store') }}"
            ...
            <div>
                ...
                <textarea name="description" rows="5" required
                          ...
                          class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    ...
                    @foreach(['low' => '🟢 Low', 'medium' => '🟡 Medium', 'high' => '🟠 High', 'urgent' => '🔴 Urgent'] as $value => $label)
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="priority" value="{{ $value }}"
                                   @checked(old('priority', 'medium') === $value)
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-400">PDF, JPG, PNG, GIF, WEBP, DOC up to 10MB</p>
            </div>
