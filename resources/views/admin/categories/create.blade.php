<x-layout title="Create Category">
    <x-slot name="header">
        <div class="space-y-2">
            ...
            <p class="text-sm text-gray-500">Add a new category for complaints.</p>
        </div>
    </x-slot>
        ...
        <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div class="flex items-center gap-3 rounded-lg border border-gray-200 bg-slate-50 px-4 py-4">
                    ...
                    <label class="flex items-center gap-2 text-sm text-gray-700">
                        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', true)) class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        Active
                    </label>
                </div>
            </div>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700">Create Category</button>
